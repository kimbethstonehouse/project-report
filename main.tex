...
Make sure you talk about multithreading not having an impact because CPU bound - multicore made the difference!

ok so
multithreading had no impact
when there is less threads than cores then cores r aat idle
performance decrease is not quite linaer - prompted to compare to linux, and then prompted to look at other ways to improve performance, experiment 1 aND 2
...
\bottomrule
\end{tabular}
\caption{Average time taken in seconds to execute the Mandelbrot benchmark within InfOS with varying configurations of cores and threads}\label{timing-infos-mb}
\end{table}

\begin{figure}[h]
    \centering
    \includegraphics[scale=0.4]{figures/mandelbrot-graph.png}
    \caption{Time taken to execute the Mandelbrot benchmark by varying numbers of cores}
...
\begin{figure}[h]
    \centering
    \includegraphics[scale=0.4]{figures/scalability-graph.png}
    \caption{Performance of InfOS vs Linux on the modified Mandelbrot benchmark with varying numbers of cores}
    \label{mandelbrot-graph}
\end{figure}
