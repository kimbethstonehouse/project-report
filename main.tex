...
Before discussing the issue of scheduling, we first define the notion of a process, which is the unit of work in an operating system. A process is a program in execution, and each process has an associated context. The context tracks the current activity of a process, and contains the program counter, register values, process state and so on. The process state may be any one of the following:
...
On a typical system, there are many processes active at a given moment in time, and the role of the scheduler is to multiplex the CPU among them. A nonpreemptive scheduler will allow the currently running process to maintain control of the CPU until it either blocks or terminates, whereas a preemptive scheduler will interrupt a running process after a given time slice to allow another process to run. The scheduler maintains a queue of ready processes, and when a CPU becomes available, it selects the next process to execute.

In the case of a single core, there are many different algorithms that can be used to choose the next process to execute, such as first-come-first-served, round-robin or priority scheduling. In the case of multiple cores, the problem becomes more complex, and there are many design decisions to be made. The scheduler may maintain a system-wide ready queue, or it may maintain a ready queue for each core. A system-wide ready queue may be better for homogeneous architectures, but care must be taken to ensure that the ready queue is not subject to race conditions (discussed further in section \ref{synchronisation}) if multiple cores were to become available at the same time. Furthermore, this introduces the issue of processor affinity. When a process has been running on a specific core, that core’s private cache will hold relevant data to that process, so the process will run faster on that specific processor. In other words, the process has an affinity for a particular core. With a system-wide ready queue, if the process were to become blocked and placed back into the ready queue, the next time it executes may be on a different core to the one it has an affinity for.
...
The scheduler only supports dispatching tasks to run on one core, so it will need to be modified to schedule tasks to run on multiple cores. As already discussed in section \ref{scheduling}, there are the issues of system-wide vs per-core ready queues, algorithms to select the next process, load balancing and processor affinity to consider.

\subsubsection{Synchronisation}
Care needs to be taken around the issue of synchronisation when running multiple processes on multiple cores, as discussed in section \ref{synchronisation}. Locking primitives such as mutex locks and semaphores can be used to ensure data integrity, but these must be designed and planned.
