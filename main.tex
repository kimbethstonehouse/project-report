...
As before, the above steps needed to be done for each core, and it seemed natural to have all cores initialising themselves in parallel. However, since each AP accesses the device manager during initialisation and uses the PIT to calibrate its timer, my implementation needed to synchronise access to these objects. To further complicate matters, the BSP uses the PIT while waiting for an AP to set the ready flag, so a previously started AP could be using the PIT to calibrate while the BSP is spinning waiting for the AP to come online. I initially tried protecting the PIT and the device manager with mutex locks. On trying to acquire a lock that is already held, the thread will yield the CPU and check again when it next runs. However, the cores were not set up to do this yet ....
Went for simple spinlock, busy wait while doing. Could have set this up to have mutexes to slightly improve efficiency since CPU not busy waiting,  but would have required rearchitecture of the system and possibly sacrifice readability, which we are deliberately aiming for
...
\section{Comparison}
todo: Implementation comparison with Linux - main differences is processes & threads vs tasks, modulariation of scheduleing policies is the same, but core load balancing policies is monolithic in linux. Oh and ours is nice and OO and C++ and pretty (sorry linus). Will do this once implementation is written. \\
Then compare performance \\

\section{Conclusion}
Main discussion: did we achieve our final goals?
Yes, it's nicely desgined nad achives same performance as Linux. 
