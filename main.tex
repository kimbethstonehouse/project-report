...
todo: define CPU vs I/O bound
explain user mode vs kernel mode?

Here, we define a process to be a program in execution. Each process has an associated context, which tracks the current activity of the process, containing the program counter, register values, process state and so on. The process state may be any one of the following:
...
The current approach to multicore scheduling is one runqueue per core that tracks and schedules the runnable tasks assigned to that core. The issue of load balancing between cores was discussed in section \ref{scheduling-challenges}, and the Linux kernel handles this by calling an \verb|idle_balance()| function when a core's runqueue becomes empty. The main scheduler also checks regularly how the task load is spread throughout the system and performs load balancing if necessary \cite{seeker}.

Todo: more about LOAD BALANCING
... bad


\subsubsection{Performance implications}
todo: a decade of wasted cores

\subsubsection{Readability}
The open-source nature of Linux makes it a good educational resource, with all source code being publicly available on GitHub \cite{linux-github}. However, the large and complex nature of the project makes it almost impossible for a novice programmer to understand. As an illustration, the graph shown in figure \ref{linux-growth} shows the growth of the source code with each release, and as of January 2020, the kernel comprised 27.8 million lines of code \cite{linux-loc}.
...
Maybe?
The implications of retrospectove muilticroe has been recognised by others ...........

\subsection{RTOS}
Another class of OS that demands performance guarantees


\section{Motivation} \label{infos-introduction}
